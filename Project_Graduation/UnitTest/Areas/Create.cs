...
public class Create
{
    ...
    //[Fact]
    //public async Task CreateAreaAsync_ShouldThrowArgumentNullException_WhenDtoIsNull()
    //{
    //    // Arrange
    //    CreateAreaDto createAreaDto = null;

    //    // Act & Assert
    //    await Assert.ThrowsAsync<ArgumentNullException>(() => _areaService.CreateAreaAsync(createAreaDto));
    //}
    [Fact]
    ...
    //    //    .ReturnsAsync(true); // Area already exists
    ...
    public async Task CreateAreaAsync_ShouldReturnSuccess_WhenAreaIsCreatedSuccessfully()
    {
        ...
        _mockRestaurantRepository.Setup(repo => repo.GetByIdAsync(It.IsAny<int>()))
            .ReturnsAsync(restaurant);
        //_mockAreaRepository.Setup(repo => repo.AnyAsync(It.IsAny<Expression<Func<Area, bool>>>()))
        //    .ReturnsAsync(false); // Area does not exist
